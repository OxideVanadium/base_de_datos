...
where p.tipo='alumno' and p.sexo='F' and g.nombre='Grado en Ingeniería Informática (Plan 2015)';
...
```sql
select d.* from departamento as d, profesor as pr
where d.id=pr.id_departamento
and pr.id_profesor is null;
```
...
where a.id_profesor=p.id_profesor and p.id_departamento=d.id
and a.curso is null;
```
...
```sql
select g.nombre, ifnull(count(a.id),0) as asignaturas from grado as g
...
```sql
select g.nombre, count(a.id) as asignaturas from grado as g
join asignatura as a on g.id=a.id_grado
group by id_grado
...
```sql
select g.nombre, a.tipo, sum(a.creditos) from grado as g
join asignatura as a on g.id=a.id_grado
group by a.tipo;
┌─────────────────────────────────────────────┬─────────────┬─────────────────┐
│                   nombre                    │    tipo     │ sum(a.creditos) │
├─────────────────────────────────────────────┼─────────────┼─────────────────┤
